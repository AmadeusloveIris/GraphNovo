...
class ReplayMemoryDataset(Dataset):
    def __init__(self, capacity: int, env_interactor, aa_dict, data, psm_head, tokenize_aa_dict, hidden_size=768, max_len=50, max_point=256, nhead=12):
        super().__init__()
        self.capacity = capacity
        self.env_interactor = iter(env_interactor)
        ...
        self.memory_pool_initialize()

    def memory_pool_initialize(self):
        self.memory = pd.DataFrame(columns=['Annotated Sequence','Inference Sequence','m/z [Da]','Charge','Experiment','File ID','First Scan', 'Loss'])
        while len(self.memory)<self.capacity:
            pep_finish_pool = next(self.env_interactor)
            temp = self.psm_head.iloc[[pep_status.idx for pep_status in pep_finish_pool]].copy()
            temp['Inference Sequence'] = [pep_status.train_seq for pep_status in pep_finish_pool]
            temp['Loss'] = [pep_status.max_loss for pep_status in pep_finish_pool]
            self.memory = pd.concat([self.memory,temp])
        self.memory = self.memory.sort_values('Loss',ascending=False)
        self.memory = self.memory.iloc[:self.capacity]

    def memory_pool_update(self):
        pep_finish_pool = next(self.env_interactor)
        temp = self.psm_head.iloc[[pep_status.idx for pep_status in pep_finish_pool]].copy()
        temp['Inference Sequence'] = [pep_status.train_seq for pep_status in pep_finish_pool]
        temp['Loss'] = [pep_status.max_loss for pep_status in pep_finish_pool]
    ...
    def memory_loss_update(self, idx, max_loss):
        max_loss = max_loss.cpu().numpy()
        self.memory = self.memory.reset_index(drop=True)
        self.memory.loc[idx, 'Loss'] = max_loss
        self.memory = self.memory.sort_values('Loss',ascending=False)
        self.memory = self.memory.iloc[:self.capacity]
    
    def __getitem__(self, idx):
        label_seq, inference_seq, moverz, charge, experiment, file_id, scan, _ = self.memory.iloc[idx]
    ...
    def peptide_mass_embedding(self, seq, moverz, charge):
        ...
        nterm_mass = np.array([0]+nterm_mass[:-1])
