...
def combination_composition_builder(max_mass: float):
    ...
    for aa in aa_list:
        ...
        for i in range(1,int(max_mass/Residual_seq(aa).mass)+1):
            ...
            if aa_composition in result: result[aa_composition] += [i*aa]
            else: result[aa_composition] = [i*aa]
            for candidate_composition in sorted_composition_list:
                ...
                if next_composition in result: result[next_composition] += [i*aa+peptide for peptide in knapsack_composition[candidate_composition]]
                else: result[next_composition] = [i*aa+peptide for peptide in knapsack_composition[candidate_composition]]
