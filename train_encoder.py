...
spec_header = pd.read_csv('./pretrain_data_sparse/genova_psm.csv', index_col='index')
...
class GenovaCollator(object):
    ...
    def encoder_collate(self, encoder_records):
        ...
        for i, record in enumerate(encoder_records):
            ...
            node_feat.append(
                pad(record['node_feat'], [0, 0, 0, max_subgraph_node - node_subgraph_node, 0, max_node - node_num]))
            node_sourceion.append(
                pad(record['node_sourceion'], [0, max_subgraph_node - node_subgraph_node, 0, max_node - node_num]))
            rel_mask.append(pad(pad(record['rel_mask'], [0, max_node - node_num], value=-float('inf')),
                                [0, 0, 0, max_node - node_num]))
            dist.append(pad(record['dist'], [0, max_node - node_num, 0, max_node - node_num]))
            charge.append(record['charge'])
            rel_coor_cated.append(
                torch.stack([i * max_node ** 2 + record['rel_coor'][0] * max_node + record['rel_coor'][1],
                             record['rel_coor'][-2] * 100 + record['rel_coor'][-1]]))
            node_mask[i, node_num:] = True

        drctn = torch.zeros(max_node, max_node) + torch.tril(2 * torch.ones(max_node, max_node), -1) + torch.triu(
            torch.ones(max_node, max_node), 1)
